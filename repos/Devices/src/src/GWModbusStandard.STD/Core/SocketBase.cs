...
namespace IoTClient
{
    ...
    public abstract class SocketBase
    {
        ...
        protected readonly object lockObject = new object(); // 使用私有锁对象
        ...
        /// <returns></returns>
        protected async Task<Result<byte[]>> SocketRead(Socket socket, int receiveCount)
        {
        ...
        /// <returns></returns>
        public abstract Task<Result<byte[]>> SendPackageSingle(byte[] command);
        ...
        public async Task<Result<byte[]>> SendPackageReliable(byte[] command)
        {
            try
            {
                var result = await SendPackageSingle(command);
                //if (!result.IsSucceed)
                //{
