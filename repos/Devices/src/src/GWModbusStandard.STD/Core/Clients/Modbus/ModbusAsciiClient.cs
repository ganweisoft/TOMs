...
namespace IoTClient.Clients.Modbus
{
    ...
    public class ModbusAsciiClient : ModbusSerialBase, IModbusClient
    {
        ...
        /// <returns></returns>
        public override async Task<Result<byte[]>> Read(string address, byte stationNumber = 1, byte functionCode = 3, ushort readLength = 1, bool byteFormatting = true)
        {
            ...
            try
            {
                ...
                //发送命令并获取响应报文
                var sendResult = await SendPackageReliable(finalCommand);
        ...
        public override async Task<Result> Write(string address, bool value, byte stationNumber = 1, byte functionCode = 5)
        {
            if (isAutoOpen) Connect();
        ...
        /// <param name="functionCode"></param>
        /// <returns></returns>
        public override async Task<Result> Write(string address, byte[] values, byte stationNumber = 1, byte functionCode = 16, bool byteFormatting = true)
        {
            if (isAutoOpen) Connect();
