...
{
    public class SerialPort4Linux : ICommunication,IDisposable
    {
        int equipno;
        EquipItem Equip;
        public int EquipNo
        ...
        int commWaitTime = 100;//默认值

        private RJCP.IO.Ports.SerialPortStream MyserialPort = null;
        string[] ss = RJCP.IO.Ports.SerialPortStream.GetPortNames();
        string portName;
        string param, timeparam;

        bool bSerialError = false;//串口是否有错误


        public SerialPort4Linux()
        {
            if (MyserialPort==null)
                MyserialPort = new RJCP.IO.Ports.SerialPortStream();
        }
        ...
        public RJCP.IO.Ports.SerialPortStream GetSerialPort()
        {
            return MyserialPort;
        }
        ...
        public bool Initialize(EquipItem item)
        {
            ...
            MyBufLen = 0;
            */
            portName = item.Local_addr;// GetPortNm(istano, iequipno);
            ...
            if (VerifyParam(param))
            {
                ...
                switch (ParamArray[2])
                {
                    ...
                    //    break;
                    case "1":
                        MyserialPort.StopBits = RJCP.IO.Ports.StopBits.One;
                        break;
                    case "1.5":
                        MyserialPort.StopBits = RJCP.IO.Ports.StopBits.One5;
                        break;
                    case "2":
                        MyserialPort.StopBits = RJCP.IO.Ports.StopBits.Two;
                        break;
                    default:
                        return false;
                }
                ...
                {
                    case "NO":
                        MyserialPort.Parity = RJCP.IO.Ports.Parity.None;
                        break;
                    case "EVEN":
                        MyserialPort.Parity = RJCP.IO.Ports.Parity.Even;
                        break;
                    case "ODD":
                        MyserialPort.Parity = RJCP.IO.Ports.Parity.Odd;
                        break;
                    case "MARK":
                        MyserialPort.Parity = RJCP.IO.Ports.Parity.Mark;
                        break;
                    case "SPACE":
                        MyserialPort.Parity = RJCP.IO.Ports.Parity.Space;
                        break;
                    default:
                        return false;
                }
        ...
        {
            if (!ss.Contains(portName))//com口不存在，直接返回
                return;
